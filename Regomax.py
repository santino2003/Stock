...
def main():
    ...
    def llmar_esc():
        ...
        raiz5.iconbitmap("R.ico")
        raiz5.focus()
        raiz5.grab_set()
        

        #label regen
        texto_escan=Label(raiz5,text="Cliente",font="14")
        ...
        cuadro_escan.focus_set()
            ...
            #boton aceptar
        def cod():
            dato = cuadro_escan.get()
            if dato == "":
                messagebox.showerror(message="El campo no puede estar vacio")
            else:
                raiz5.destroy() 
                escanear(dato)
        ...
        bot_a_prodcep= Button(raiz5, text="ACEPTAR", width=10, command=cod)
        bot_a_prodcep.place(x=75,y=75)
    ...
    def escanear(cliente):
        ...
        def cod():
            ...
            if rg.entregar_producto(cod,dato):
    ...
    #boton escanear
    bot_entregar= Button(miFrame, text="ESCANEAR", width=15, command=llmar_esc)
    bot_entregar.place(x=7,y=140)

    def copia_archivo():
        rg.descargar_datos_a_csv("stock","stock.csv","fecha")

    def copia_stock():
        rg.descargar_datos_a_csv("ultimas_24h","logisticas.csv","fecha_entrega")
    def copiar_entregado():
            rg.descargar_datos_a_csv("entregados","despachados.csv","fecha_entrega")

    

    #boton copia stok
    bot_entregar= Button(miFrame, text="DESC S", width=15, command=copia_archivo)
    ...
    bot_entregar= Button(miFrame, text="LOGISTICA", width=15, command=copia_stock)
    bot_entregar.place(x=130,y=260)

    bot_entregar= Button(miFrame, text="DESPACHADO", width=15, command=copiar_entregado)
    bot_entregar.place(x=130,y=290)
    ...
    def ventana_de_modificacion(cod):
        ...
        def mod_prod():
            ...
            rg.modificar_archivos(cod,nuevo_producto)
            raiz7.destroy()
